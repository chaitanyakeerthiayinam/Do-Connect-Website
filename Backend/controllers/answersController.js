...
exports.getAnswerById = async (req, res) => {
    try {
        ...
        res.status(500).send({ message: error });// Handle any server errors by responding with a 500 status
    }
};
...
exports.getNumOfLikesForAnswer = async (req, res) => {
    try {
        ...
        res.status(200).send({ message: `No of likes: ${answer.likes.length}` });// Respond with a success message and the updated answer
    } catch (error) {
        res.status(500).send({ message: error });
    }
};
